...
/**
 * Wires banking-service HTTP app with Inversify, matching auth-service shape.
 */
export class BootstrapApp {
  private readonly container: Container;
  private readonly configService: BankingConfigService;
  private app: Application | null = null;

  constructor(container = new InversifyContainer().getContainer()) {
    this.container = container;
    this.configService = this.container.get<BankingConfigService>(TYPES.ConfigService);
  }
  ...
  getServer(): Application {
    if (!this.app) {
      throw new Error("Banking service has not been set up yet");
    }
